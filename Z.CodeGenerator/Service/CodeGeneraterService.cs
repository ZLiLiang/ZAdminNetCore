...
namespace Z.CodeGenerator.Service
{
    public class CodeGeneraterService : DbProvider
    {
        ...
        public List<DbTableInfo> GetAllTables(string dbName, string tableName, PagerInfo pager)
        {
            ...
            return tableList.Skip(pager.PageNum * (pager.PageNum - 1))
                .Take(pager.PageSize)
                .OrderBy(f => f.Name)
                .ToList();
        }
        ...
        public DbTableInfo GetTableInfo(string dbName, string tableName)
        {
            ...
            if (!string.IsNullOrEmpty(tableName))
            {
                return tableList.Where(f => f.Name.Equals(tableName, StringComparison.OrdinalIgnoreCase))
                    .FirstOrDefault();
            }
            return null;
        }
