...
using SqlSugar;
using Z.CodeGenerator.Model;
using Z.Common;
using Z.Model.System.Generate;

namespace Z.CodeGenerator
{
    /// <summary>
    /// 代码生成器
    /// </remarks>
    ...
    public class CodeGeneratorTool
    {
        ...
        public static void Generate(GenerateDto dto)
        {
            ...
            dto.VueParentPath = dto.VueVersion == 3 ? "ZAdmin-vue" : "Z.Vue";
            if (!vuePath.IsEmpty())
            {
            ...
            var cloumns = dto.GenTable.Columns;

            replaceDto.PKName = cloumns.Find(f => f.IsPk || f.IsIncrement)?.CsharpField ?? "Id";
            replaceDto.PKType = cloumns.Find(f => f.IsPk || f.IsIncrement)?.CsharpType ?? "int";

            replaceDto.UploadFile = cloumns.Any(f => f.HtmlType.Equals(GenConstants.HTML_IMAGE_UPLOAD) || f.HtmlType.Equals(GenConstants.HTML_FILE_UPLOAD)) ? 1 : 0;
            replaceDto.SelectMulti = cloumns.Any(f => f.HtmlType.Equals(GenConstants.HTML_SELECT_MULTI)) ? 1 : 0;
            replaceDto.ShowEditor = cloumns.Any(f => f.HtmlType.Equals(GenConstants.HTML_EDITOR)) ? 1 : 0;
        ...
        private static GenTableColumn InitColumnField(GenTable genTable, DbColumnInfo column, List<OracleSeq> seqs, OptionsSetting optionsSetting)
        {
            ...
            if (optionsSetting.Gen.GenDbConfig.DbType == 3)
            {
                ...
                var isIdentity = seqs.Any(predicate => seqName.Equals(predicate.SEQUENCE_NAME, StringComparison.CurrentCultureIgnoreCase));
            ...
            GenTableColumn genTableColumn = new()
            {
                ...
                IsInsert = !column.IsIdentity || GenConstants.inputDtoNoField.Any(predicate => predicate.Contains(column.DbColumnName, StringComparison.OrdinalIgnoreCase)),//非自增字段都需要插入
                IsEdit = true,
                IsQuery = false,
                IsExport = true,
            ...
            if (GenConstants.imageFiled.Any(predicate => column.DbColumnName.ToLower().Contains(predicate.ToLower())))
            {
                genTableColumn.HtmlType
        ...
        private static void InitJntTemplate(GenerateDto dto,ReplaceDto replaceDto)
        {
...
#endif
            dto.GenTable.Columns = dto.GenTable.Columns.OrderBy(keySelector => keySelector.Sort).ToList();
            bool showCustomInput = dto.GenTable.Columns.Any(predicate => predicate.HtmlType.Equals(GenConstants.HTML_CUSTOM_INPUT, StringComparison.OrdinalIgnoreCase));
            ...
            dicts.AddRange(dto.GenTable.Columns.FindAll(match => dictHtml.Contains(match.HtmlType)));
            if (dto.GenTable.SubTable != null && dto.GenTable.SubTable.IsNotEmpty())
            {
