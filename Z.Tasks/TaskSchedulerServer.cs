        ...
        private ITrigger CreateSimpleTrigger(SysTasks tasksQz)
        {
            if (tasksQz.RunTimes > 0)
            {
                ITrigger trigger = TriggerBuilder.Create()
                    ...
                    .WithSimpleSchedule(action => action.WithIntervalInSeconds(tasksQz.IntervalSecond)
                    .WithRepeatCount(tasksQz.RunTimes))
                    .ForJob(tasksQz.ID, tasksQz.JobGroup)
                    .Build();
                return trigger;
            }
            else
            {
                ITrigger trigger = TriggerBuilder.Create()
                    ...
                    .StartAt(tasksQz.BeginTime.Value)
                    .EndAt(tasksQz.EndTime.Value)
                    .WithSimpleSchedule(action => action.WithIntervalInSeconds(tasksQz.IntervalSecond).RepeatForever())
                    .ForJob(tasksQz.ID, tasksQz.JobGroup)
                    .Build();
                return trigger;
            }
