...
namespace Z.Admin.WebApi.Middleware
{
    /// <summary>
    /// 全局异常处理中间件
    ...
    public class GlobalExceptionMiddleware
    {
        ...
        public GlobalExceptionMiddleware(RequestDelegate next, ISysOperLogService sysOperLogService)
        {
            this.next = next;
        ...
        private async Task HandleExceptionAsync(HttpContext context, Exception exception)
        {
            ...
            {
                code = (int)ResultCode.GLOBAL_ERROR;
            ...
            SysOperLog sysOperLog = new()
            {
                Status = 1,
                OperIp = ip,
                OperUrl = HttpContextExtension.GetClientUserIp(context),
            ...
            LogEventInfo logEvent = new(logLevel, "GlobalExceptionMiddleware", error)
            {
                Exception = exception,
                Message = error
            };
            logEvent.Properties["status"] = 1;//走正常返回都是通过走GlobalExceptionFilter不通过
            logEvent.Properties["jsonResult"] = responseResult;
            logEvent.Properties["requestParam"] = sysOperLog.OperParam;
            logEvent.Properties["user"] = sysOperLog.OperParam;
