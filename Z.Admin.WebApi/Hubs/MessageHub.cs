...
namespace Z.Admin.WebApi.Hubs
{
    ...
    public class MessageHub : Hub
    {
        ...
        public static readonly List<OnlineUsers> clientUsers = new List<OnlineUsers>();
        private readonly Logger logger = LogManager.GetCurrentClassLogger();
        private readonly ISysNoticeService sysNoticeService;

        public MessageHub(ISysNoticeService sysNoticeService)
        {
        ...
        {
            var user = clientUsers.Where(p => p.ConnnectionId == Context.ConnectionId)
                .FirstOrDefault();
            //判断用户是否存在，否则添加集合
            if (user != null)
            {
