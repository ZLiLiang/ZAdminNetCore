...
namespace Z.Admin.WebApi.Framework
{
    /// <summary>
    /// 2020-11-20
    /// </summary>
    public class JwtUtil
    {
        ...
        public static LoginUser? ValidateJwtToken(IEnumerable<Claim> jwtToken)
        {
            try
            {
                ...
                var userData = jwtToken.FirstOrDefault(predicate => predicate.Type == ClaimTypes.UserData)?.Value;
                if (userData != null)
                {
                    ...
                    {
                        permissions=new List<string> { GlobalConstant.AdminRole };
                    }
                    if (permissions == null)
                    {
                        return null;
