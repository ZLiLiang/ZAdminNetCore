...
using Z.CodeGenerator;
using Z.CodeGenerator.Model;
using Z.CodeGenerator.Service;
using Z.Common;
using Z.Model;
...
namespace Z.Admin.WebApi.Controllers.System
{
    ...
    public class CodeGeneratorController : BaseController
    {
        ...
        public CodeGeneratorController(IGenTableService genTableService, IGenTableColumnService genTableColumnService, ISysMenuService sysMenuService, IWebHostEnvironment webHostEnvironment)
        {
        ...
        /// 查询生成表数据
        /// </summary>
        /// <param name="tableMame"></param>
        /// <param name="pagerInfo"></param>
        /// <returns></returns>
        [HttpGet("list")]
        [ActionPermissionFilter(Permission = "tool:gen:list")]
        public IActionResult GetGenTable(string? tableMame, PagerInfo pagerInfo)
        {
            //查询原表数据，部分字段映射到代码生成表字段
            var rows = _genTableService.GetGenTables(new GenTable
            {
                TableName = tableMame,
            }, pagerInfo);
        ...
        public IActionResult ImportTableSave(string tables,string dbName)
        {
            ...
            DbConfigs dbConfigs = AppSettings.Get<DbConfigs>(nameof(GlobalConstant.CodeGenDbConfig));
            ...
            foreach (string tableName in tableNames)
            {
                ...
                if (tabInfo != null)
                {
                    List<OracleSeq> seqs = new List<OracleSeq>();
                    ...
                    if (genTable.TableId > 0)
                    {
                        ...
                        _genTableColumnService.DeleteGenTableColumnByTableName(tableName);
        ...
        public IActionResult CodeGenerate([FromBody ] GenerateDto dto)
        {
            ...
            //生成压缩包
            string zipReturnFileName = $"ZAdmin.NET-{genTableInfo.TableComment}-{DateTime.Now:MMddHHmmss}.zip";

            //生成代码到指定文件夹
            CodeGeneratorTool.Generate(dto);
            if (genTableInfo.Options.GenerateMenu)
            {
                _sysMenuService.AddSysMenu(genTableInfo, dto.ReplaceDto.PermissionPrefix, dto.ReplaceDto.ShowBtnEdit, dto.ReplaceDto.ShowBtnExport);
            }
            ...
            return SUCCESS(new
            {
                path = "/Generatecode/" + zipReturnFileName,
                fileName = dto.ZipFileName
            });
        }
        ...
        public IActionResult SyncDb(string tableName, long tableId = 0)
        {
