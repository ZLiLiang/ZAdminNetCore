...
namespace Z.Admin.WebApi.Controllers.System
{
    ...
    public class TasksLogController : BaseController
    {
        ...
        public IActionResult GetList([FromQuery] PagerInfo pager, [FromQuery] TasksLogQueryDto queryDto)
        {
            ...
            var predicate = Expressionable.Create<SysTasksLog>()
                .And(it => it.CreateTime >= queryDto.BeginTime && it.CreateTime <= queryDto.EndTime)
                .AndIF(queryDto.JobName.IfNotEmpty(), m => m.JobName.Contains(queryDto.JobName))
                .AndIF(queryDto.JobGroup.IfNotEmpty(), m => m.JobGroup == queryDto.JobGroup)
                .AndIF(queryDto.Status.IfNotEmpty(), m => m.Status == queryDto.Status)
                .AndIF(queryDto.JobId.IfNotEmpty(), m => m.JobId == queryDto.JobId);
